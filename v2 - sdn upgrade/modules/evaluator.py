...
def parse_evaluator_logs(filename):
    ...
    pattern = re.compile(
        r"\[(.*?)\] \[Evaluator\] Diversity: ([\d.]+), Redundancy: ([\d.]+), Shuffle Efficiency: ([\d.]+), Energy: (\d+)"
    )
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if match:
                ...
                diversity_values.append(float(match.group(2)))
                redundancy_values.append(float(match.group(3)))
                efficiency_values.append(float(match.group(4)))
                energy_values.append(int(match.group(5)))
...
if __name__ == "__main__":
    ...
    if ts:
        ...
        print("No [Evaluator] logs found in the log file.")
